        ...
        try {
            switch (rq.getParameter("action")) {
                case "modifymeet":
                    ...
                    rs.sendRedirect(rq.getContextPath() + "/views/users/admin/confirmpage.jsp?action=1");
                    break;
                case "ripmeet":
                    int MeetID = Integer.parseInt(rq.getParameter("MeetID"));
                    rq.getSession().setAttribute("MeetID", MeetID);
                    rs.sendRedirect(rq.getContextPath() + "/views/users/admin/confirmpage.jsp?action=2");
                    break;
                case "modifybell":
                    CreateObjectBell(rq, rs);
                    rs.sendRedirect(rq.getContextPath() + "/views/users/admin/confirmpage.jsp?action=3");
                    break;
                case "ripbell":
                    int BellID = Integer.parseInt(rq.getParameter("BellID"));
                    rq.getSession().setAttribute("BellID", BellID);
                    rs.sendRedirect(rq.getContextPath() + "/views/users/admin/confirmpage.jsp?action=4");
                    break;
